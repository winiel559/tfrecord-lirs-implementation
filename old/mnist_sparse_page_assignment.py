...
# unzip page_order to instance_order
def unzip_order(page_order, index_order, oft):
    ...
    for page_idx in page_order:
        # put all instances of the chosen page contiguously
        start_idx = page_aligned_indices[page_idx]
        if(page_idx+1 < len(page_aligned_indices)):
            end_idx = page_aligned_indices[page_idx+1]
        else:
            end_idx = NUM_INSTANCE-1
        #print("put ", start_idx, "to", end_idx)
...
if __name__ == '__main__':
    ...
    n_page = int(os.path.getsize(
        DATA_DIR + "mnist_sparse_padded_BFD.tfrecords") / PAGESIZE) + 1
    training_page_order = np.zeros(
        (NUM_THREADS, n_page)).astype(np.int32)
    training_order = np.zeros((NUM_THREADS, NUM_INSTANCE)).astype(np.int32)
    for i in range(NUM_THREADS):
        for j in range(n_page):
            training_page_order[i][j] = j

    # Create the training file list
    dataset_list = []
    oft_list = []
    for i in range(1):
        dataset_list.append(DATA_DIR + "mnist_sparse_padded_BFD.tfrecords")
        oft_list.append(DATA_DIR + "mnist-sparse_padded_BFD-offset_table")
    # Reader threads list
    threads = []
    ...
    for k in range(NUM_THREADS):
        random.shuffle(training_page_order[k])

    oft = open(oft_list[0], 'rb')
    # unzip page_order to instance_order
    for k in range(NUM_THREADS):
        unzip_order(training_page_order[k], training_order[k], oft)
